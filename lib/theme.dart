...
ThemeData buildAppTheme(Brightness brightness) {
  ...
  final scheme = ColorScheme.fromSeed(seedColor: Colors.black, brightness: brightness)
      .copyWith(
    primary: fg,
    onPrimary: bg,
    secondary: fg,
  ...
  return ThemeData(
    ...
    switchTheme: SwitchThemeData(
      thumbColor: WidgetStateProperty.resolveWith(
        (states) => states.contains(WidgetState.selected) ? fg : (isLight ? Colors.white : Colors.black),
      ),
      trackColor: WidgetStateProperty.resolveWith(
        (states) => states.contains(WidgetState.selected) ? fg.withValues(alpha: 0.5) : Colors.grey,
      ),
    ),
  );
}
